...
import { deleteDoc, ensureSeedDocuments, putDoc, MAX_DOC_SIZE } from "@/lib/doc-store";
...
function genererRecordsDemo(e: Etudiant): PaiementMensuel[] {
  ...
  return mois.map((m, i) => {
    ...
    return {
      ...
      datePaiement: statut === "paye" ? `${annee}-${String(moisCal).padStart(2, "0")}-05` : "",
...
function load(): Snapshot {
  ...
  try {
    ...
    if (!Array.isArray(parsed.groupConfigs))
      parsed.groupConfigs = structuredClone(DEFAULT_GROUP_CONFIGS);
    for (const e of parsed.etudiants) {
      ...
      if (!Array.isArray(e.paiementsMensuelsRecords) || e.paiementsMensuelsRecords.length === 0) {
    ...
    if (storedLundi && storedLundi !== expectedLundi) {
      const hasSeedIds = parsed.seances.some((s) => /^se-\d+-\d+$/.test(s.id));
      if (hasSeedIds) {
...
type IstpmCtx = {
  ...
  updateGroupConfig: (
    id: string,
    patch: { name?: string; semester?: string; studentCount?: number },
  ) => void;
  ...
  updateStructureAccueil: (
    oldName: string,
    body: { nouveauNom?: string; capacite?: number },
  ) => void;
...
export function IstpmProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const prefer = <T,>(local: T[], remote: T[]): T[] => (local.length > 0 ? local : remote);
  ...
  const updateEtudiant = useCallback((id: string, patch: Partial<Etudiant>) => {
    apiUpdateEtudiant(id, patch as unknown as Record<string, unknown>).catch(() => {});
    setSnap((s) => ({
      ...s,
      etudiants: s.etudiants.map((e) => (e.id === id ? { ...e, ...patch } : e)),
    }));
  }, []);

  const deleteEtudiant = useCallback((id: string) => {
  ...
  const restoreEtudiant = useCallback((id: string) => {
  ...
  const addFormateur = useCallback((data: NouveauFormateur) => {
  ...
  const updateFormateur = useCallback((id: string, patch: Partial<Formateur>) => {
    apiUpdateFormateur(id, patch as unknown as Record<string, unknown>).catch(() => {});
    setSnap((s) => ({
      ...s,
      formateurs: s.formateurs.map((f) => (f.id === id ? { ...f, ...patch } : f)),
    }));
  }, []);

  const deleteFormateur = useCallback((id: string) => {
  ...
  const archiveFormateur = useCallback(
    (
      ...
      setSnap((s) => {
        let formateurs = s.formateurs.map((f) => (f.id === id ? { ...f, archived: true } : f));
        ...
        for (const r of groupReassignments) {
          ...
          if (target && !target.groupes.includes(r.groupName)) {
            formateurs = formateurs.map((f) =>
              f.id === r.targetFormateurId ? { ...f, groupes: [...f.groupes, r.groupName] } : f,
            );
          }
          formateurs = formateurs.map((f) =>
            f.id === id ? { ...f, groupes: f.groupes.filter((g) => g !== r.groupName) } : f,
          );
        }
        return { ...s, formateurs };
      });
  ...
  const restoreFormateur = useCallback((id: string) => {
    apiRestoreFormateur(id).catch(() => {});
    setSnap((s) => ({
      ...s,
      formateurs: s.formateurs.map((f) => (f.id === id ? { ...f, archived: false } : f)),
    }));
  }, []);
  ...
  const updateGroupConfig = useCallback(
    (id: string, patch: { name?: string; semester?: string; studentCount?: number }) => {
      ...
      setSnap((s) => ({
        ...s,
        groupConfigs: s.groupConfigs.map((g) => (g.id === id ? { ...g, ...patch } : g)),
      }));
    },
    [],
  );

  const deleteGroupConfig = useCallback((id: string) => {
  ...
  const updateExamen = useCallback((id: string, patch: Partial<Examen>) => {
  ...
  const deleteExamen = useCallback((id: string) => {
  ...
  const attachDocument = useCallback(async (examenId: string, file: File) => {
    if (file.size > MAX_DOC_SIZE) {
      throw new Error(`Fichier trop volumineux (max ${Math.round(MAX_DOC_SIZE / 1024 / 1024)} Mo)`);
    }
    ...
    setSnap((s) => {
      ...
      return {
        ...s,
        examens: s.examens.map((x) => (x.id === examenId ? { ...x, document: meta } : x)),
      };
    });
  }, []);
  ...
  const removeDocument = useCallback(
    async (examenId: string) => {
      ...
      setSnap((s) => ({
        ...s,
        examens: s.examens.map((x) => (x.id === examenId ? { ...x, document: undefined } : x)),
      }));
  ...
  const saveNotesExamen = useCallback((examenId: string, saisies: SaisieNote[]) => {
    ...
    setSnap((s) => {
      ...
      const etudiants = s.etudiants.map((e) => {
        ...
        const note = Math.round((parts.reduce((a, b) => a + b, 0) / parts.length) * 100) / 100;

        const existante = e.notes.find((n) => n.module === examen.module);
        const notes = existante
          ? e.notes.map((n) => (n.module === examen.module ? { ...n, note } : n))
  ...
  const updateBulletin = useCallback((id: string, patch: Partial<Bulletin>) => {
    apiUpdateBulletin(id, patch as unknown as Record<string, unknown>).catch(() => {});
    setSnap((s) => ({
      ...s,
      bulletins: s.bulletins.map((b) => (b.id === id ? { ...b, ...patch } : b)),
    }));
  }, []);

  const publierBulletin = useCallback((id: string) => {
      ...
      setSnap((s) => {
        const updatedRecords =
          s.etudiants
            ...
            ?.paiementsMensuelsRecords.map((r) => (r.id === id ? { ...r, ...patch } : r)) ?? [];
        const allPaye =
          updatedRecords.length > 0 && updatedRecords.every((r) => r.statut === "paye");
  ...
  const addNote = useCallback((etudiantId: string, note: NoteModule) => {
    ...
    setSnap((s) => {
      ...
      const notes = existante
        ? etudiant.notes.map((n) => (n.module === saved.module ? saved : n))
  ...
  const addFiliere = useCallback((nom: string) => {
  ...
  const deleteFiliere = useCallback((nom: string) => {
  ...
  const addStructureAccueil = useCallback((nom: string, capacite = 5) => {
  ...
  const deleteStructureAccueil = useCallback((nom: string) => {
  ...
  const updateModule = useCallback((id: string, data: Omit<ModuleRecord, "id">) => {
    setSnap((s) => ({
      ...s,
  ...
  const updateSeance = useCallback((id: string, patch: Partial<Seance>) => {
  ...
  const deleteSeance = useCallback((id: string) => {
  ...
  const dashboard = useMemo(() => {
    const inscrits = snap.etudiants.filter((e) => e.statut === "inscrit" || e.statut === "diplome");
    const notes = snap.etudiants.filter((e) => e.moyenne > 0);
    return {
      totalInscrits: inscrits.length,
      deltaSemestre: 6,
      formateursActifs: snap.formateurs.filter((f) => f.statut !== "en_conge").length,
      tauxReussite: notes.length
        ? Math.round((notes.filter((e) => e.moyenne >= 10).length / notes.length) * 100)
        : 0,
  ...
  const etudiantsARisque = useMemo(
    () => snap.etudiants.filter((e) => (e.moyenne > 0 && e.moyenne < 10) || e.statut === "abandon"),
    [snap.etudiants],
  );
  ...
  const aTraiter = useMemo(
    () => ({
      ...
      bulletinsAPublier: snap.bulletins.filter((b) => b.statut !== "publie").length,
  ...
  const reussiteFiliere = useMemo(
    () =>
      FILIERES.map((f) => {
        const inscrits = snap.etudiants.filter((e) => e.filiere === f && e.moyenne > 0);
        const reussite = inscrits.length
          ? Math.round((inscrits.filter((e) => e.moyenne >= 10).length / inscrits.length) * 100)
          : 0;
